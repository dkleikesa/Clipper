...
/**
 * 预览滑出 / 收回时，窗口几何过渡的时长。**当前为 0：不做过渡，一次到位。**
 *
 * 过渡的本意是盖掉「系统把上一帧内容合成进新窗口」那一帧（收起时旧帧左边正好是预览，补到
 * 新窗口位置上，看起来就是预览盖住了主列表）。实测下来它没盖住那一帧，反而引入了新问题：
 * 窗口逐帧变宽 / 变窄期间，卡片与主列表会有一两帧按不同步的宽度排版（右侧停靠时表现为
 * 「预览把主列表挤小，随后又被放大」）。所以默认关掉；把这里改回正数即可打开。
 */
