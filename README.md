...
**顺带一提**：五档搜索命中高亮样式可选、类型筛选与四种排序、`⏎` 直接粘贴 / `⌘⏎` 去格式粘贴、主题三模式、面板宽度与预览开合状态持久化——这些细节都有。
...
- 置顶：固定在列表顶部或底部，顺序由**置顶时间**决定——不随排序设置变，也不会因为你用了它（粘贴 / 再复制一次）而重排

**操作**

- 全局热键呼出面板（默认 `⇧⌘C`），可自定义或清除绑定
- 激活选中项的四种按法各是一条可录制快捷键：激活 `⌥⏎`、去格式激活 `⌥⌘⏎`、直接粘贴 `⏎`、去格式粘贴 `⌘⏎`；鼠标单击做什么可单独指定（默认激活）
- 置顶 `⌥P`、删除 `⌥⌫`、预览开关 `⌃Space`
