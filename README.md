...
* 置顶：置顶项固定在最上方或最下方并**常驻在滚动区之外**（Maccy 的 `HistoryListView`），前九个置顶项按显示顺序自动占用 `⌘1`…`⌘9`；可在偏好设置里删除（不再分配字母键）
...
* **快捷键可自定义**：呼出 / 暂停记录 / 置顶 / 删除 / 预览五个快捷键都能在偏好设置里重新录制；✕ 清除绑定（不是恢复默认，默认值由设置页底部的「恢复默认设置」负责）；录制时会先检查这段组合能不能用——裸键、与其它功能重复、面板内置按键、被系统或别的应用占用都会被拒绝并说明原因，**录制不会因此退出**，直接换一个组合接着按即可（Esc 取消）；**录制期间其它快捷键一律不响应**（系统级热键会临时注销再注册回来），按键只归录制器
...
* **忽略**：只保留「暂停记录新的复制」；剪贴板声明的临时 / 机密 / 自动生成类型（`org.nspasteboard.*`）始终不记录，不提供配置
* 单条删除、清空未置顶、全部清除（每次都弹出二次确认）
* 存储：历史上限（按未置顶记录的**条数**计，默认 200 条，条数不设上限，置顶项不受限）、可分别开关文本/图片/文件入库、输入框下方实时显示当前条数与**数据库文件大小**、剪贴板检查间隔可调；**删除累计超 1 MB 回收空闲页，清空数据与退出时 `VACUUM` 压紧数据库文件**
* 外观：弹窗位置（光标 / 菜单栏图标 / 屏幕中心）、**弹窗屏幕（多显示器）**、置顶位置、菜单栏图标显示开关、显示页脚/搜索框/色块/特殊符号/应用图标、图片最大高度、**窗口尺寸（拖动边缘调整，可一键恢复默认）**
* 置顶项管理：纯文本项可直接编辑内容；选中一行后按 `Delete` 即可删除（Maccy 的 `PinsSettingsPane.onDeleteCommand`）
* 高级：退出时清空历史、开机自启（macOS `SMAppService`）
...
| 暂停 / 恢复记录 | `⌘P`（可自定义；与设置页里的「暂停记录新的复制」开关是同一项） |
| 下一个 / 上一个 | `↓`（`⇧↓` 同效）/ `↑`（`⇧↑` 同效） |
...
只有「呼出面板」由 Carbon 注册为系统级热键，会**全系统截获**该组合（不论当前在哪个应用）；
其余可录制快捷键（暂停记录、置顶、删除、预览）都只在面板拿到键盘焦点时生效。
置顶项的快捷键角标会随修饰键在 `⌘x` / `⌥x` / `⌘⇧x` 之间切换——与 Maccy 相同；未置顶条目没有快捷键。
...
| 忽略规则 | 忽略正则 / 忽略应用 / 忽略剪贴板类型都能在设置里配置 | 只保留「暂停记录新的复制」；`org.nspasteboard` 的临时 / 机密 / 自动生成类型始终忽略，但不可配置 |
| 历史上限 | `size` 按**条数**计（默认 200） | 同为按条数计，默认 200，且不设最大条数 |
...
| 偏好设置窗口 | 独立的设置窗口，6 个分页（General / Storage / Appearance / Pins / Ignore / Advanced）+ 工具栏图标 | 单个可滚动对话框，按同样的分组组织；忽略分区收敛为「暂停记录新的复制」 |
