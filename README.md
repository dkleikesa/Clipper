...
* 置顶：置顶项固定在最上方或最下方并**常驻在滚动区之外**（Maccy 的 `HistoryListView`），前九个置顶项按显示顺序自动占用 `⌘1`…`⌘9`；可在偏好设置里改别名 / 删除（不再分配字母键）
...
* 置顶项管理：改别名，纯文本项还可直接编辑内容；选中一行后按 `Delete` 即可删除（Maccy 的 `PinsSettingsPane.onDeleteCommand`）
