...
/**
 * 历史列表与页脚的键盘 / 鼠标导航，以及选中集的增删。
 *
 * 键盘导航只在内容区（历史 + 置顶）内移动，页脚只由鼠标悬停高亮。
 *
...
internal class HistoryNavigationController(
    ...
    /**
     * 最近一次导航输入是否来自键盘：键盘移动置 `true`，任何鼠标输入置 `false`。
     * 只在本类内部参与判断，界面从不渲染，因此留在这里而不是 [ClipboardUiState]。
     */
     ...
     *
     * 两种情形下**不动选中集**，都是为了保住 `⌘` / `⇧` 点击的基准：已有多选时划过不该把它
     * 冲掉；正按住选中修饰键时，鼠标不能先把基准换掉（否则「选好第 9 条，再 `⌘` 点第 13 条」
     * 会因为划过第 13 行而变成空操作）。
     *
     * 悬停**不**递增 `historyScrollToken`：行只高亮、列表不动，否则会出现「悬停 → 选中变化 →
     * 滚动 → 鼠标下换了行」的循环。
     */
    ...
    /** 移到列表会重新启用键盘导航（`selectHistory`，顺带折叠到单选）；页脚内部移动只动高亮。 */
...
/** 把纯规则算出的结果写回状态；[scroll] 决定界面要不要把光标行滚进可视区。 */
